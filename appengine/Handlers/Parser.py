...
import json
import cgi		
from Handlers import DBHandler #All the methods to access the DB
...
class JSON_parser(webapp2.RequestHandler):
	...
	def post(self):		
		try:		
			...
			if not DBHandler.alta_sensor(str(jdata["Tipo"]),float(jdata["Valor"]), str(jdata["Ubicacion"])): ##Just for tests, real timestamp is given by DB
